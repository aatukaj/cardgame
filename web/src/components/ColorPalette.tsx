        ...
        <UICard className={className}>
            ...
            <UICard.Body>
                {isOpen ? <div className="grid grid-cols-2 w-12">
                    {AVATAR_COLORS.map(({ bg }, i) => <div key={i} onClick={(e) => { e.preventDefault(); onClick(i); setOpen(false) }} className={twJoin(bg, "size-6 cursor-pointer")}></div>)}
                </div> : <button onClick={(e) => { e.preventDefault(); setOpen(true) }} className={twJoin("w-12 h-6", AVATAR_COLORS[selected].bg)}></button>}
            </UICard.Body>
        </UICard>
    )
}
